...
The most important thing about the XAA is that:

- IdP (Governance) knows the relationship
- IdP can run the access controls of the relationship
...
- We can let AI Agent to lower down the scope of the exchange so that AI agent doesn't do unrequested actions

Can explain the following flow:

![jag_sequence_diagram](./assets/jag_sequence_diagram.png)

Can explain the following edge cases:

- What if AI agent asks for the delete JAG, even if the user's intent (prompt) is to read?
  - This is big, but again:
    - Risk-based Auth: May ask once again user if the agent can delete it (Prompt)
    - Okta: Okta may prompt user to accept the deletion
    - Okta: In the first place Okta may have the setting default `delete` permission not given (No prompt)
- What if MCP Server returns the Fully authorized AT?
  - As a client perspective, it is NOT their business to know the AT is fully authorized or not. They just use it
  - As a server perspective, it is their responsibility to handle the correctly permissioned AT to client
- What if the Access Token lasts for a long time, but we've disallowed between the IdP and MCP server?
  - At this point there is no way to handle this, and to do this basically we need a shorter AT.
