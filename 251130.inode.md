...
- [Dive in](#dive-in)
  ...
  - [What's inside the inode](#whats-inside-the-inode)
    ...
    - [Can I create a custom hard link for the directory?](#can-i-create-a-custom-hard-link-for-the-directory)
      - [Why not allowed?](#why-not-allowed)
    - [Can I create a custom hard link for a file then?](#can-i-create-a-custom-hard-link-for-a-file-then)
    - [Can I create a custom soft link for a file or directory?](#can-i-create-a-custom-soft-link-for-a-file-or-directory)
      - [Softlink's inode is different from the original file's inode](#softlinks-inode-is-different-from-the-original-files-inode)
...
> [!INFO]
> We can learn that it is technically possible to have multiple hard links for a directory.
...
### Can I create a custom hard link for the directory?

Can we create a custom hard link for the directory, just like a file?
```sh
...
### Can I create a custom hard link for a file then?

Yes!

```sh
ln sample_file.txt sample_hard_link_file.txt
stat -x sample_hard_link_file.txt
#   File: "sample_hard_link_file.txt"
#   Size: 13           FileType: Regular File
#   Mode: (0644/-rw-r--r--)         Uid: (  501/     ajk)  Gid: (   20/   staff)
# Device: 1,17   Inode: 110676931    Links: 2
# Access: Sun Nov 30 14:26:15 2025
# Modify: Sun Nov 30 13:47:48 2025
# Change: Sun Nov 30 14:26:14 2025
#  Birth: Sun Nov 30 13:47:48 2025
```


### Can I create a custom soft link for a file or directory?

Yes!

For file:
```sh
ln -s sample_file.txt sample_soft_link_file.txt
stat -x sample_soft_link_file.txt
#   File: "sample_soft_link_file.txt"
#   Size: 15           FileType: Symbolic Link
#   Mode: (0755/lrwxr-xr-x)         Uid: (  501/     ajk)  Gid: (   20/   staff)
# Device: 1,17   Inode: 110698860    Links: 1
# Access: Sun Nov 30 20:23:02 2025
# Modify: Sun Nov 30 20:23:02 2025
# Change: Sun Nov 30 20:23:02 2025
#  Birth: Sun Nov 30 20:23:02 2025
```

#### Softlink's inode is different from the original file's inode

For `sample_file.txt`:
```sh
stat -x sample_file.txt | grep Inode
# Device: 1,17   Inode: 110676931    Links: 2
```

For `sample_soft_link_file.txt`:
```sh
stat -x sample_soft_link_file.txt | grep Inode
# Device: 1,17   Inode: 110698860    Links: 1
```
...
So the soft link itself is a unique data stored in the file system, separate from the original file. The soft link contains a reference (the path) to the original file, but it is treated as a distinct entity with its own inode.

![soft_link_distinct_inode](/assets/soft_link_distinct_inode.png)
